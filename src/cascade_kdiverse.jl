function cascade_kdiverse(Asort,koutmean,kinmean,ksd,gprim,sigma,tmax)
    ...
    #ksd is really kvariance!
    koutdist = Normal(koutmean,ksd)
    kindist = Normal(kinmean,ksd)
    ...
    for t=2:tmax
        ...
        for i=1:n
            ...
            kout = abs.(rand(koutdist,length(nn_out[i])))
            kin = abs.(rand(kindist,length(nn_in[i])))
            ...
            #Spin function
            newstate = sign(past_s[i] - sum(kout .* past_s[nn_out[i]]) + sum(kin .* past_s[nn_in[i]]) + sigma*rand(epsilondist) + g);
